...
# after revisions and tweaking parameters and changing model, final result of trianing

# config

CSV_DIR   = Path(r"etfs")
files = os.listdir("etfs")
SYMBOLS   = [file[:file.index(".csv")] for file in files]
SYMBOLS = SYMBOLS[::300]
print(len(SYMBOLS))
LAG_DAYS  = 15         # ≈ 3 trading weeks
SEED      = 42
N_SPLITS  = 5
...
# load, tag symbol id, global sort
frames = []
...
# build X, y with 15‑day lag‑stacking
X_rows, y_rows = [], []
for i in range(LAG_DAYS, len(data) - 1):          # need t & t+1
